...
def compare_predictions(pred_rust, pred_r, y_true, y_data):
    ...
    if pred_r is not None:
        ...
        print(f"  Correlation:    {corr:.6f} {'✓ Excellent' if corr > 0.99 else '⚠ Check'}")
        print(f"  RMSE diff:      {rmse_diff:.6f} {'✓ Good' if rmse_diff < 0.1 else '⚠ Check'}")
        print(f"  Max diff:       {max_diff:.6f} {'✓ Good' if max_diff < 0.2 else '⚠ Check'}")
...
def visualize_results(X, y, y_true, pred_rust, pred_r, effects):
    ...
    ax1.scatter(range(len(y)), y[idx_sort], alpha=0.3, s=10, label='Data', color='lightgray')
    ax1.plot(range(len(y_true)), y_true[idx_sort], 'k--', linewidth=1.5, label='True', alpha=0.6)
    ax1.plot(range(len(pred_rust)), pred_rust[idx_sort], 'b-', linewidth=2, label='mgcv_rust', alpha=0.7)
    if pred_r is not None:
        ax1.plot(range(len(pred_r)), pred_r[idx_sort], 'r--', linewidth=2, label='R mgcv', alpha=0.7)
    ...
    if pred_r is not None:
        ...
        ax2.plot([lim_min, lim_max], [lim_min, lim_max], 'r--', linewidth=2, label='Perfect agreement')
        ...
        ax3.axvline(x=np.mean(diff), color='r', linestyle='--', linewidth=2, label=f'Mean: {np.mean(diff):.6f}')
    ...
    for i in range(4):
        ...
        ax.scatter(X[idx_sort_x, i], y[idx_sort_x], alpha=0.2, s=10, label='Data', color='lightgray')
        ax.plot(X[idx_sort_x, i], true_effects[i][idx_sort_x], 'k-', linewidth=2, label='True effect', alpha=0.7)
...
def test_extrapolation(gam_rust, X_train, n_test=200):
    ...
    print(f"Prediction range: [{pred_rust_extrap.min():.4f}, {pred_rust_extrap.max():.4f}]")
    ...
    if HAS_RPY2:
        ...
        with localconverter(ro.default_converter + numpy2ri.converter):
            ...
            ro.r(f'pred_r_extrap <- predict(gam_fit, newdata=data.frame({newdata_str}))')
...
def main():
    ...
    X, y, y_true, effect_1, effect_2, effect_3, effect_4 = generate_4d_data(n, noise_level)
    ...
    if result_r is not None:
        ...
        if isinstance(lambda_rust, (int, float)):
            ...
            for i in range(len(lambda_rust)):
                ratio = lambda_rust[i] / lambda_r[i] if lambda_r[i] > 0 else float('inf')
                print(f"x{i+1:<14} {lambda_rust[i]:<15.6f} {lambda_r[i]:<15.6f} {ratio:<10.4f}")
