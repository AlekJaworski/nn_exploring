...
use mgcv_rust::{Family, OptimizationMethod, SmoothTerm, SmoothingParameter, GAM};
use ndarray::Array1;
...
fn test_case(x_vals: &[f64], y_vals: &[f64], k: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let penalty_rank = BlockPenalty::new(
        gam_rank.smooth_terms[0].penalty.clone(),
        0,
        gam_rank.smooth_terms[0].penalty.nrows(),
    );
    let penalties_rank = vec![penalty_rank];
    sp_rank.optimize(
        &y_arr,
        &x_2d,
        &Array1::ones(y_vals.len()),
        &penalties_rank,
        20,
        1e-6,
    )?;
    ...
    println!(
        "  Rank-based:  λ = {:.6}, time = {:.2}ms",
        lambda_rank,
        rank_time.as_secs_f64() * 1000.0
    );
    ...
    let start = std::time::Instant::now();
    let penalty_edf = BlockPenalty::new(
        gam_edf.smooth_terms[0].penalty.clone(),
        0,
        gam_edf.smooth_terms[0].penalty.nrows(),
    );
    let penalties_edf = vec![penalty_edf];
    sp_edf.optimize(
        &y_arr,
        &x_2d,
        &Array1::ones(y_vals.len()),
    ...
    println!(
        "  EDF-based:   λ = {:.6}, time = {:.2}ms",
        lambda_edf,
        edf_time.as_secs_f64() * 1000.0
    );
    println!(
        "  Difference:  λ_ratio = {:.4}, time_ratio = {:.2}x",
        lambda_edf / lambda_rank,
        edf_time.as_secs_f64() / rank_time.as_secs_f64()
    );
