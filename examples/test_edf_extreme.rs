...
//! Requires the `blas` feature: cargo run --example test_edf_extreme --features blas

#[cfg(feature = "blas")]
fn main() -> Result<(), Box<dyn std::error::Error>> {
    use mgcv_rust::block_penalty::BlockPenalty;
    use mgcv_rust::reml::{penalty_sqrt, ScaleParameterMethod};
    use mgcv_rust::{OptimizationMethod, SmoothTerm, SmoothingParameter, GAM};
    use ndarray::Array1;
    ...
    println!("\nPenalty matrix: {}x{}", penalty.nrows(), penalty.ncols());
    println!("Penalty rank: {}", penalty_sqrt(penalty)?.ncols());

    let bp = BlockPenalty::new(penalty.clone(), 0, penalty.nrows());
    ...
    let result_rank = sp_rank.optimize(&y_arr, &x_2d, &w, &[bp.clone()], 20, 1e-6);
    let time_rank = start.elapsed();

    match result_rank {
        Ok(_) => {
            println!("Converged!");
            println!("  lambda = {:.6}", sp_rank.lambda[0]);
    ...
    let start = std::time::Instant::now();
    let result_edf = sp_edf.optimize(&y_arr, &x_2d, &w, &[bp], 20, 1e-6);
    let time_edf = start.elapsed();

    match result_edf {
        Ok(_) => {
            println!("Converged!");
            println!("  lambda = {:.6}", sp_edf.lambda[0]);
            println!("  Time: {:.2}ms", time_edf.as_secs_f64() * 1000.0);
            println!(
                "  Overhead: {:.1}x",
                time_edf.as_secs_f64() / time_rank.as_secs_f64()
            );
        }
        Err(ref e) => {
            println!("Failed: {}", e);
    ...
    println!(
        "Rank-based method: {}",
        if rank_success { "SUCCESS" } else { "FAILED" }
    );
    println!(
        "EDF-based method:   {}",
        if edf_success { "SUCCESS" } else { "FAILED" }
    );

    if rank_success && edf_success {
        let lambda_ratio = sp_edf.lambda[0] / sp_rank.lambda[0];
        println!("Lambda ratio (EDF/Rank): {:.3}", lambda_ratio);
        println!(
            "Time ratio (EDF/Rank): {:.2}",
            time_edf.as_secs_f64() / time_rank.as_secs_f64()
        );
    }
    ...
    if edf_success && !rank_success {
        ...
        println!("\nBoth methods failed - may need different approach for k=200, n=50");
    }
...
fn main() {
    println!("Run with: cargo run --example test_edf_extreme --features blas");
}
