...
fn main() {
    ...
    println!("Performance:");
    println!(
        "  Uncached (with eigendecomp): {:.3}s per call",
        time_uncached / 10.0
    );
    println!(
        "  Cached (no eigendecomp):     {:.3}s per call",
        time_cached / 10.0
    );
    ...
    println!(
        "  Precomp once + 10 cached:    {:.3}s per call (amortized)",
        amortized
    );
    println!(
        "  vs Uncached:                 {:.3}s per call",
        time_uncached / 10.0
    );
    println!(
        "  Savings:                     {:.3}s per call\n",
